...
class NewsProcessor:
    ...
    def __init__(self, hf_api_token, news_api_key, *args, **kwargs):
...
class FinancialProcessor:
    ...
    def get_financial_data(self, ticker_symbol: str):
        ...
        # Format the data
        financial_data = {
            "revenue": { str(year): int(value) for year, value in revenue.items() if pd.notna(value) },
            "net_income": {str(year): int(value) for year, value in net_income.items() if pd.notna(value)}
        }
